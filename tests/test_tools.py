...
class TestContentReadTool:
    @pytest.mark.asyncio
    async def test_reads_from_state(self):
        from src.tools.implementations.content_io import ContentReadTool
...
class TestContentListTool:
    @pytest.mark.asyncio
    async def test_lists_content(self):
        from src.tools.implementations.content_io import ContentListTool

        tool = ContentListTool()
        ctx = ToolContext(
            session_id="s1",
            working_dir="/tmp",
            project_state={
        ...
        assert len(items) == 4  # 3 channels + review
        assert items[0]["has_content"] is True
        assert items[2]["has_content"] is False  # empty xhs
